...
# inds_orig <- c(inds_old[1:5],inds_old[25],inds_old[6:24]) #%# DELETE
...
load(file=paste0('data/IC_networks_ordered_',nQ,'.RData'))
xii$meta$cifti$names <- paste0('IC', IC_order)
...
# FC1 <- FC1[reorder,reorder,,] #%# DELETE
# FC2 <- FC2[reorder,reorder,,] #%# DELETE
FC1 <- FC1[IC_order, IC_order, , ]
FC2 <- FC2[IC_order, IC_order, , ]
mat2UT <- function(x){
  x[upper.tri(x)]
}
