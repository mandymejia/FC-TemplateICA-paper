...
if(run_template){
  ...
  #%# Creates Fig. F.12
  if(make_images){
    view_xifti_surface(template_exp$mean, idx=c(1, 2, 8, 10, 16), zlim=c(-lim, lim), fname = file.path('templates',paste0('template',nQ),'template_mean'))
    view_xifti_surface(template_exp$var, idx=c(1, 2, 8, 10, 16), zlim=c(0,lim^2), fname = file.path('templates',paste0('template',nQ),'template_var'), color_mode='sequential')
  }
}
load(file='subjects3.RData')
template <- readRDS(file.path(main_dir,'templates',paste0('template_',nQ,'.rds')))
