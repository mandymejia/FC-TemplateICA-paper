...
if(first_run){
  ...
  # BOLD_fnames <- paste0('data_longsim/subj',1:500,'.dtseries.nii')  Moved to 0_setup.R
...
load(file = 'results/long_duration/MSE_true_prior.RData') 

NN <- 50 # Number of sbujects
...
durations <- c(200, 400, 800, 1200, 1600, 2400) 
pdf(file.path('..', 'response', 'MSE_TCs.pdf'), width=6, height=4)
aa <- 1
...
pdf(file.path('..', 'response', 'MSE_truth_prior.pdf'), width=8, height=6)
par(mfrow=c(1,2))
for(aa in 1:2){
