...
save(ICs, Q, GICA, N, file = 'GICA.RData')

if(first_run){
  plot(template_mean, idx=1:5, zlim=c(-0.5,0.5),
       title = paste0('Generating Mean, IC ', 1:5),
       fname = paste0('templates/generating_mean', 1:5)) #%# Creates (part of) Fig. D.1

  plot(template_var, idx=1:5, zlim=c(0,0.05), 
       title = paste0('Generating Var, IC ', 1:5),
       fname = paste0('templates/generating_var', 1:5)) #%# Creates (part of) Fig. D.1
}
...
if(first_run){
  ...
  plot(tmean_est_xifti, idx=1:5, zlim=c(-0.5,0.5),
       ...
       fname = paste0('templates/template_mean', 1:5)) #%# Creates (part of) Fig. D.3
...
if(first_run){
  ...
  pdf('templates/FCtemplate.pdf', height=5, width=5.5) #%# Creates (part of) Fig. D.4
  zlim_FC <- c(-0.8, 0.8)
  ...
  pdf('templates/FCtemplate2.pdf', height=5, width=5.5) #%# Creates (part of) Fig. D.4
  zlim_FC <- c(-0.8, 0.8)
  ...
  plot(template, fname = 'templates/template_est', idx=1:5, stat='mean', zlim=c(-0.5,0.5)) #%# Creates (part of) Fig. D.3
  plot(template, fname = 'templates/template_est', idx=1:5, stat='var', zlim=c(0,0.03)) #%# Creates (part of) Fig. D.3
...
if(first_run){
  ...
  # pdf('TCs_real/TCs_test.pdf', width=8, height=5)
  # ggplot(TCs_example, aes(x=time, y=value)) + geom_line() +
  #   facet_grid(IC ~ subject, labeller='label_both') + theme_few()
  # dev.off()
  #%# Creates (part of) Fig. D.2 
  pdf('TCs_real/TCs.pdf', width=8, height=5)
  tryCatch({
    p <- ggplot(TCs_example, aes(x=time, y=value)) +
      geom_line() +
      facet_grid(IC ~ subject, labeller='label_both') +
      ggthemes::theme_few()
    print(p)  # This is mandatory inside pdf()
  }, finally = {
    dev.off()
  })
...
if(first_run){
  ...
  pdf('plots/FC_mean_and_SD.pdf', height=5, width=5.5) #%# Creates (part of) Fig. D.4
