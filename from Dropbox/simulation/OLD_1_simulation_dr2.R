...
#devtools::install_github('mandymejia/fMRItools', ref = '0.5.0')
...
if(run_part1){
  ...
  for(ii in 50:50){
    ...
    #for(aa in 1:num_alg){ #### COMMENT OUT THIS LINE AND RUN FOR aa=1
      ...
      if(alg == 'tICA') {
        FC_est[,,ii,num_alg+1] <- cor(result_aa$result_DR$A) 
        FC_est[,,ii,num_alg+2] <- cor(result_aa$result_DR$A2) #####DR
    ...
    save(results_ii, comptime_ii, file=paste0('results/testsubj',ii,'_dr2.RData'))
