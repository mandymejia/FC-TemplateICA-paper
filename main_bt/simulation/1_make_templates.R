...
# main_dir <- '~/Dropbox (Brown)/FCTemplateICA/simulation/' #if run locally
main_dir <- '~/Documents/Github/FC-TemplateICA-paper/simulation' #if run on RED
setwd(main_dir)
source('0_setup.R')
...
output_dir <- "../results/1_make_templates"
...
# Save the data
save(ICs, Q, GICA, N, file = file.path(output_dir, "GICA.RData"))

if (first_run) {
  # Ensure the directory exists
  output_dir <- "../plots/1_make_templates/templates"
  if (!dir.exists(output_dir)) {
    dir.create(output_dir, recursive = TRUE)
  }
  
  # Plot generating mean
  plot(template_mean, idx = 1:5, zlim = c(-0.5, 0.5),
       title = paste0('Generating Mean, IC ', 1:5),
       fname = paste0(output_dir, "/generating_mean", 1:5))
  
  # Plot generating variance
  plot(template_var, idx = 1:5, zlim = c(0, 0.05),
       title = paste0('Generating Var, IC ', 1:5),
       fname = paste0(output_dir, "/generating_var", 1:5))
}
...
if(first_run){
  ...
  for(ii in 1:n){
    ...
    if(ii %in% 1:3){
      # Ensure the directory exists
      output_dir <- "../plots/1_make_templates/exampleICs"
      if (!dir.exists(output_dir)) {
        dir.create(output_dir, recursive = TRUE)
      }
      
      # Generate xifti objects
      ...
      plot(subjICs_xifti, idx = 1:5, zlim = c(-0.5, 0.5),
           ...
           fname = paste0(output_dir, '/subject', ii, '_IC', 1:5),
           hemisphere = 'left')
      
      # Plot deviations from the mean
      plot(dev_xifti, idx = 1:5, zlim = c(-0.1, 0.1),
           title = paste0('Subject ', ii, ', Deviation ', 1:5),
           fname = paste0(output_dir, '/subject', ii, '_dev', 1:5),
           hemisphere = 'left')
      
    }
  }
  saveRDS(subjICs, '../results/1_make_templates/subjICs.RDS')
}
subjICs <- readRDS('../results/1_make_templates/subjICs.RDS')

# for David : This is a VxQ "S" matrix and surface mesh
...
if(first_run){
  ...
  TCs <- readRDS('~/Documents/Github/FC-TemplateICA-paper/results/TCs.RDS') #TxQxn (n=subjects) -- 1113 sessions
  ...
  output_file <- path.expand("../plots/1_make_templates/TCs_real/")
  ...
  pdf(file.path(output_file, "TCs.pdf"), width = 8, height = 5)
  ...
  pdf('../plots/1_make_templates/TCs_real/TCs_real_FC.pdf', height=5, width=5.5)
  ...
  #save time courses
  TC1 <- TCs[,,1:n]
  save(TC1, file='../results/1_make_templates/TCs_sim.RData')
}
load(file='../results/1_make_templates/TCs_sim.RData')
TC_sim <- TC1 # for David : These are the TxQ "A" matrices for 550 subjects
...
if(first_run){
  ...
  pdf('../plots/1_make_templates/FC_mean_and_SD.pdf', height=5, width=5.5)
...
output_file <- '/N/project/FCTemplateICA/data/1_make_templates'

if (!dir.exists(output_file)) {
  dir.create(output_file, recursive = TRUE)
}
BOLD_fnames <- paste0('/N/project/FCTemplateICA/data/1_make_templates',1:n,'.dtseries.nii')
...
if(first_run){
  ...
  BOLD_fnames_train <- BOLD_fnames[1:500]
    ...
    output_file <- "../results/1_make_templates/templates/template.RDS"
  ...
  pdf_file <- "../plots/1_make_templates/templates/FCtemplate.pdf"
  ...
  # Open PDF device
  pdf(pdf_file, height = 5, width = 5.5)
  zlim_FC <- c(-0.8, 0.8)
  ...
  # The error occurs because the code attempts to access template$template$FC_Chol, which does not exist in the 
  # template object. As a result, template_FC_mean2 and template_FC_var2 are assigned NULL, leading to failure when 
  # plotting. This likely stems from outdated or copied code referencing a non-existent FC_Chol entry. Since FC_Chol 
  # is not defined or generated anywhere in the script, the block is invalid.
  ...
  pdf('../plots/1_make_templates/templates/FCtemplate2.pdf', height=5, width=5.5)
  zlim_FC <- c(-0.8, 0.8)
  ...
  plot(template, fname = '../plots/1_make_templates/templates/template_est', idx=1:5, stat='mean', zlim=c(-0.5,0.5))
  plot(template, fname = '../plots/1_make_templates/templates/template_est', idx=1:5, stat='var', zlim=c(0,0.03))
  plot(newdata_xifti(GICA, template$template$varNN/template_var_est), zlim=c(1,5), fname = '../plots/1_make_templates/templates/template_var_bias')

  #SMALL BIAS IN TEMPLATE MEAN? probably due to dual regression
  pdf('../plots/1_make_templates/template_mean_bias.pdf', width=5, height=5)
