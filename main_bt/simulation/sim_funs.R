...
# FUNCTION TO VISUALIZE FC MATRICES
#x = FC matrix
#zlim = color scale limits
plot_FC <- function(x, zlim=c(-1,1), title=NULL, cols=c('darkblue','turquoise','white','pink','red'), break_by=0.5, digits_legend=1, cor=TRUE, box=FALSE, box_col='black'){
  ...
  if(box){
    segments(0.5, 2.5, 3.5, 2.5, col=box_col, lwd=5)
    segments(0.5, 5.5, 3.5, 5.5, col=box_col, lwd=5)
    segments(0.5, 2.5, 0.5, 5.5, col=box_col,lwd=5)
    segments(3.5, 2.5, 3.5, 5.5, col=box_col,lwd=5)
  }
  # color scale
  par(mar=c(1,1,0,3))
